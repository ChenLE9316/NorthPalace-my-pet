...
# Remove the differently-shaped service convenience transport after the base patch removes the
# PersistenceHandle variant.
replace(
...
# ---------------------------------------------------------------------------
# Durable memory origin domain contract.
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Evaluator: manual/automatic is independent from nullable activity FK.
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Memory Browser records expose durable origin; manual CRUD always remains manual.
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# SQLite schema v3: existing pre-evaluator rows are safely classified as manual.
# ---------------------------------------------------------------------------
...
# Low-level MemoryDraft insertion remains a test fixture; production evaluator owns automatic writes.
replace(
    "src-tauri/src/persistence.rs",
...
# ---------------------------------------------------------------------------
# Frontend provenance is explicit rather than inferred from a nullable activity FK.
# ---------------------------------------------------------------------------
replace(
...
# ---------------------------------------------------------------------------
# Target harness must parse in Windows PowerShell 5.1 (the target baseline tool requirement).
# ---------------------------------------------------------------------------
replace(
...
print("Pre-AI semantic hardening v2 applied.")
